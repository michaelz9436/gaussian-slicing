...
// =================================================================================
// 核心修改: 最终版的 preprocess_slice_backward_cuda 核函数
// =================================================================================
__global__ void preprocess_slice_backward_cuda(
    int P,
    const float3* means,
    const int* radii,
    const float* opacities,
    ...
    bool antialiasing)
{
    ...
    float dist_z = abs(p_orig.z - z_position);
    float scale_z = scales[idx].z * scale_modifier;
    float z_variance = scale_z * scale_z + 1e-9;
    float z_attenuation = exp(-0.5f * dist_z * dist_z / z_variance);
    const float dL_dopacity_in = dL_dopacity[idx];
    dL_dopacity[idx] = dL_dopacity_in * z_attenuation;

    // --- 3. 3D协方差梯度 ---
    const float* cov3D = cov3Ds + 6 * idx;
    float c_xx = cov3D[0], c_xy = cov3D[1], c_yy = cov3D[3];
    constexpr float h_var = 0.3f;
    c_xx += h_var; c_yy += h_var;

    float denom = c_xx * c_yy - c_xy * c_xy;
    float denom2inv = 1.0f / ((denom * denom) + 0.0000001f);
    
    float3 dL_dconic_v = { dL_dconic[4 * idx], dL_dconic[4 * idx + 1], dL_dconic[4 * idx + 3] };
    
    float dL_dc_xx = denom2inv * (-c_yy * c_yy * dL_dconic_v.x + 2 * c_xy * c_yy * dL_dconic_v.y + (denom - c_xx * c_yy) * dL_dconic_v.z);
    float dL_dc_yy = denom2inv * (-c_xx * c_xx * dL_dconic_v.z + 2 * c_xx * c_xy * dL_dconic_v.y + (denom - c_xx * c_yy) * dL_dconic_v.x);
    float dL_dc_xy = denom2inv * 2 * (c_xy * c_yy * dL_dconic_v.x - (denom + 2 * c_xy * c_xy) * dL_dconic_v.y + c_xx * c_xy * dL_dconic_v.z);

    atomicAdd(&dL_dcov3D[6 * idx + 0], dL_dc_xx);
    atomicAdd(&dL_dcov3D[6 * idx + 1], dL_dc_xy);
    atomicAdd(&dL_dcov3D[6 * idx + 3], dL_dc_yy);
    ...
    if (x_range == 0.0f || y_range == 0.0f) return;

    float dpx_dx = W / x_range;
    float dpy_dy = H / y_range;
    float dL_dx = dL_dmean2D[idx].x * dpx_dx;
    float dL_dy = dL_dmean2D[idx].y * dpy_dy;
    
    atomicAdd(&dL_dmeans[idx].x, dL_dx);
    atomicAdd(&dL_dmeans[idx].y, dL_dy);

    // b. Z分量
    float dL_d_z_attenuation = dL_dopacity_in * opacities[idx];
    float sign_z = (p_orig.z > z_position) ? 1.0f : -1.0f;
    float d_z_attenuation_dz = z_attenuation * (-dist_z / z_variance) * sign_z;
    float dL_dz = dL_d_z_attenuation * d_z_attenuation_dz;
    atomicAdd(&dL_dmeans[idx].z, dL_dz);

    // c. 尺度(scale)的Z分量梯度
    float d_z_attenuation_d_scale_z = z_attenuation * (0.5f * dist_z * dist_z / (z_variance * z_variance)) * (2.0f * scale_z);
    float dL_d_scale_z = dL_d_z_attenuation * d_z_attenuation_d_scale_z;
    atomicAdd(&dL_dscale[idx].z, dL_d_scale_z);
}
...
// =================================================================================
// 核心修改 2: 创建新的反向传播预处理函数的C++包装器
// =================================================================================
void BACKWARD::preprocess_slice(
    int P,
    const float3* means,
    const int* radii,
    const float* opacities,
    ...
    bool antialiasing)
{
    preprocess_slice_backward_cuda << <(P + 255) / 256, 256 >> > (
        P,
        means,
        radii,
        opacities,
